...
public ActionResult Create(Complaint newComplaint)
{
    if (ModelState.IsValid)
    {
        // Check if the ExecutiveID is provided
        if (newComplaint.ExecutiveID == 0) // Assuming 0 is the default value for ExecutiveID
        {
            ModelState.AddModelError("ExecutiveID", "Executive ID is required");
            var executives = _db.Executives
                .Select(e => new SelectListItem
                {
                    Value = e.ExecutiveID.ToString(),
        ...
        {
            ModelState.AddModelError("ExecutiveID", "Selected Executive not found");
        }
    }

    // If the model state is not valid or the executive wasn't found, retrieve executives again and return to the view
    var executivesList = _db.Executives
        .Select(e => new SelectListItem
        {
            Value = e.ExecutiveID.ToString(),
            Text = e.ExecutiveName
    ...
    ViewBag.Executives = executivesList;
    return View(newComplaint);
}
